...
# Vim-style navigator: / searches; b/w/i/d filters state; a shows all.
...
# State filter and text query are separate. Session-name hits keep every child
...
# pane edge — $E[K clears to the default background, not this one. Returns
# through $line, like color_dot returns through $dot.
# Cursor row: the state's own hue, dark enough to sit just off the terminal
# background — 24-bit because the 256-colour cube bottoms out far brighter than
# this. Terminals without RGB downsample. A shade down when $2 is empty (the
# sidebar is not focused). Sets $row_bg.
state_bg() {
	case "$1${2:+-focused}" in
	blocked-focused) row_bg=$'\033[48;2;42;16;16m' ;; # red
	blocked) row_bg=$'\033[48;2;32;12;12m' ;;
	working-focused) row_bg=$'\033[48;2;38;32;16m' ;; # yellow
	working) row_bg=$'\033[48;2;29;24;12m' ;;
	*-focused) row_bg=$'\033[48;2;15;36;16m' ;;       # green: idle and done
...
EOF
	...
	cols="${cols:-30}"
	[ -n "$rows" ] || rows="$(tput lines 2>/dev/null)"
	rows="${rows:-24}"
	...
	# Filters ride header; hint uses existing second row so click coordinates stay.
	...
	room=$((cols - 6 - NOTICE_LEN))
	[ "$room" -lt 0 ] && room=0
	filter_info="${filter_info:0:room}"
	if [ -n "$search_focused" ]; then
		header_hint='↑↓/^n/^p move · ^u clear · esc back'
	elif [ -n "$HINT" ]; then
		header_hint='u update · / search'
	else
		header_hint='/ search · b/w/i/d/a states'
	fi
	header_hint="${header_hint:0:cols}"
	# focused sidebar wears a title bar: bright-black background comes from the
	# terminal palette, so it tracks the theme. Padded spaces carry it to the
	# pane edge ($E[K would clear to the default background instead)
	hdr="" pad=""
	if [ -n "${AGENTS_MON_PIN:-}" ] || [ "$active" = "${TMUX_PANE:-}" ]; then
		hdr="$BAR_BG"
		width=$((cols - 6 - ${#filter_info} - NOTICE_LEN)) # "agents" + extras
		[ "$width" -gt 0 ] && pad="$(printf '%*s' "$width" '')"
	fi
	frame="$E[H$hdr$E[1magents$E[22m$E[2m$filter_info$E[22m$NOTICE$pad$E[0m$E[K$NL$E[2m$header_hint$E[0m$E[K$NL"
	...
	if [ -z "$debounced" ]; then
		...
		frame="$frame$E[2mno matches · a shows all$E[0m$E[K$NL"
	else
		while IFS=$'\t' read -r pane loc agent state cwd title; do
			[ -n "$pane" ] || continue
			if [ "${loc%%:*}" != "$session" ]; then
			...
			n=$((n + 1))
			if [ "$n" = "$sel" ]; then
				# cursor mark takes the state hue, bold only while focused
				case "$state" in
				blocked) fg=31 ;;
				working) fg=33 ;;
				*) fg=32 ;;
			...
			avail=$((cols - 6 - ${#agent})) # " ❯ ● name " prefix
...
quit() {
	[ -n "${AGENTS_MON_PIN:-}" ] && rm -f "$AGENTS_MON_PIN"
	exit 0
}
...
show_help() { # blocks until a key; animations pause meanwhile
	printf '%s' "$E[2J$E[H$E[1magents — help$E[0m $E[2m$VERSION$E[0m$NL$NL$E[1mstatus$E[0m$NL $E[32m⣿$E[0m  idle$NL $E[33m⠹$E[0m  working (spinner)$NL $E[31m⣿$E[0m  blocked, waiting for input (blinks)$NL $E[32m⣿$E[0m  done, not viewed yet (blinks)$NL$NL$E[1mkeys$E[0m$NL j/k ↑/↓  move selection$NL Enter/l  jump to agent$NL /        live text search$NL b/w/i/d  filter blocked/working/idle/done$NL a        show all agents$NL u        update to the latest release$NL q Esc    close sidebar$NL ?        this help$NL$NL$E[2mpress any key to return$E[0m"
	IFS= read -rsn1
	printf '%s' "$E[2J"
}
...
while :; do
	...
	if [ -n "$search_focused" ]; then
		if IFS= read -rsn1 -t "$READ_WAIT" key; then
			case "$key" in
			'') jump ;; # Enter switches to selected match
			...
			"$E")
				...
				'[A' | 'OA') sel=$((sel - 1)) ;;
				'[B' | 'OB') sel=$((sel + 1)) ;;
				'') search_focused="" ;; # keep query, return to navigator
				esac
				;;
			*)
	...
	if IFS= read -rsn1 -t "$READ_WAIT" key; then
		...
		l) jump ;;
		/) focus_search ;;
		b) set_state_filter blocked ;;
		w) set_state_filter working ;;
		i) set_state_filter idle ;;
		d) set_state_filter done ;;
		a) clear_filter ;;
		"$BS" | $'\010') [ -n "$state_filter" ] && clear_filter ;;
		u) update ;;
		'?') show_help ;;
		'') jump ;; # Enter
		"$E")
