...
Post-merge verification against `master` also passed. The integration preserves the all-pane sidebar inventory while caching only detected-agent screens: all 190 Rust tests, the release build, polling and navigation harnesses, and the complete shell integration runner passed. On an isolated attached tmux server with `show_all_panes = true`, a fresh real Claude pane was observed as `working` through both the public detector and runtime cache, then as `idle` through both after completion; the sidebar continued to render pane rows. The server's `@agenmux-bin` and loaded `agents/*.conf` were rechecked against this worktree before the run.
