...
if [ "$table" = agenmux ] && [ "$initial_focus" = agenmux ] &&
  ...
  [ "$settings_open" -eq 1 ] && [ "$settings_dropdown" -eq 1 ] &&
  ...
  echo "settings: open=$settings_open dropdown=$settings_dropdown cancelled=$settings_cancelled saved=$settings_saved responsive=$settings_responsive returned=$settings_returned popup=$settings_popup"
  exit 1
fi
