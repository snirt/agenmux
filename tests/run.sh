...
if [ "$fail" -eq 0 ]; then
  ...
  mk_release() {
    ...
    chmod +x "$d/$package/target/release/agenmux" \
      "$d/$package/target/release/agenmux-notifier"
...
SH
  ...
  if [ "$(engine)" = "v0.1.0" ] && [ "$(marker)" = "v0.1.0" ] &&
    [ -x "$tmp/plugin/target/release/agenmux-notifier" ] &&
    [ -f "$tmp/home/Applications/Agenmux.app/Contents/MacOS/agenmux-notifier" ] &&
    [ "$(sed -n '1p' "$tmp/plugin/target/release/.agenmux-latest")" = "v0.1.1" ] &&
    [ "$(sed -n '1p' "$tmp/plugin/target/release/.agenmux-tags")" = "v0.1.1" ]; then
  ...
  rm -f "$tmp/plugin/target/release/.agenmux-latest" \
    "$tmp/plugin/target/release/.agenmux-tags"
  printf 'v0.1.0\n-\n' >"$tmp/plugin/target/release/.agenmux-version"
  install_bin v0.1.1
  if [ "$(sed -n '1p' "$tmp/plugin/target/release/.agenmux-latest")" = "v0.1.1" ] &&
    [ "$(sed -n '1p' "$tmp/plugin/target/release/.agenmux-tags")" = "v0.1.1" ]; then
  ...
  cat >"$bad/unpack/$package/target/release/agenmux" <<SH
#!/usr/bin/env bash
touch "$tmp/executed-unverified"
SH
  chmod +x "$bad/unpack/$package/target/release/agenmux"
  tar -czf "$bad/$package.tar.gz" -C "$bad/unpack" "$package"
  printf '%064d  ./%s.tar.gz\n' 0 "$package" >"$bad/SHA256SUMS"
  set_version 0.2.0
  printf 'v0.2.0\n' >"$tmp/plugin/target/release/.agenmux-latest"
  printf '#!/usr/bin/env bash\nexit 1\n' >"$tmp/bin/cargo"
  chmod +x "$tmp/bin/cargo"
  rm -f "$tmp/plugin/target/release/agenmux" \
    "$tmp/plugin/target/release/agenmux-notifier"
  bad_rc=0
  install_bin v0.2.0 >/dev/null 2>&1 || bad_rc=$?
  if [ "$bad_rc" -ne 0 ] &&
    [ ! -e "$tmp/plugin/target/release/agenmux" ] &&
...
RS
  ...
  if PATH="$tmp/offline:$HOME/.cargo/bin:$PATH" RUSTUP_HOME="$rustup_home" \
    ...
    [ "$("$cargo_plugin/target/release/agenmux")" = cargo-fallback ]; then
...
case "$*" in
  ...
  "wait-for -L agenmux-install"|"wait-for -U agenmux-install") exit 0 ;;
esac
exit 0
SH
  chmod +x "$tmp/bin/tmux"
  if PATH="$tmp/bin:$PATH" bash "$tmp/plugin/agenmux.tmux" activate popup exact-client &&
...
SH
  ...
  if grep -Fq -- '-c popup-client' "$tmp/tmux.log" &&
    grep -Fq -- '-e AGENMUX_POPUP_CLIENT=popup-client' "$tmp/tmux.log" &&
...
SH
  ...
  touch "$tmp/pin"
  TMUX_STUB_LOG="$tmp/tmux.log" TMPDIR="$tmp" PATH="$tmp/bin:$PATH" \
    AGENMUX_PIN="$tmp/pin" TMUX_PANE="%%99" "$BIN" sidebar >/dev/null 2>&1 &
  pid=$!
  sleep 0.1
...
if [ "$fail" -eq 0 ] && command -v tmux >/dev/null && [ -x "$BIN" ]; then
  ...
  $T set-option -g @agenmux-bin "$BIN_ABS"
  env TMPDIR="$tmp" TMUX="$tmp/sock,0,0" PATH="$tmp/bin:$PATH" AGENMUX_DIR="$DIR" \
  ...
  focus_kept=0
  [ "$($T display-message -p -t t: '#{pane_title}')" != agenmux ] && focus_kept=1
  keys_ok=0
  $T list-keys -T agenmux | grep -Fq "key 'close'" && keys_ok=1
  control="$($T show-option -gqv @agenmux-control-client)"
  control_ok=0
  [ -n "$control" ] &&
  ...
  wait
  raced="$($T list-panes -t "$racew" -F '#{pane_title}' | grep -cx agenmux)"
  ...
  sleep 4
  optw2="$($T show-option -gqv @agenmux-width)"
...
if [ "$fail" -eq 0 ] && command -v tmux >/dev/null && [ -x "$BIN" ]; then
  ...
  env TMPDIR="$tmp" TMUX="$tmp/sock,0,0" PATH="$tmp/bin:$PATH" AGENMUX_DIR="$DIR" \
    "$BIN_ABS" toggle split
  sleep 2
  mirrors() { $T list-panes -a -F '#{pane_title}' 2>/dev/null | grep -cx agenmux; }
  mir="$($T list-panes -t t: -F '#{pane_id}	#{pane_title}' |
    awk -F'\t' '$2 == "agenmux" { print $1; exit }')"
  opened="$(mirrors)"
...
if [ "$fail" -eq 0 ] && [ "$(uname -s)" = Darwin ] &&
  [ -x "$DIR/target/release/agenmux-notifier" ]; then
  # install-app.sh must assemble a signed Agenmux.app around the notifier
  tmp="$(mktemp -d)"
  plist="$tmp/apps/Agenmux.app/Contents/Info.plist"
  mkdir -p "$tmp/apps/AgentsMon.app"
...
if [ "$fail" -eq 0 ]; then
  AGENMUX_BIN="$BIN" bash "$DIR/tests/daemon-orphan.sh" || fail=1
fi
exit $fail
