...
cp "$DIR/agenmux.tmux" "$DIR/Cargo.toml" "$DIR/Cargo.lock" "$plugin/"
...
run_tmux_case() {
  ...
  tmux -L "$socket" set-option -g status-right '#{agenmux}'
  tmux -L "$socket" run-shell "bash '$plugin/agenmux.tmux'"
...
run_immediate_popup_bootstrap() { # verified|cargo|bad-checksum
  ...
  local marker="$root/popup-$1/opened" socket="agenmux-popup-$1-$$"
  ...
  cp "$DIR/agenmux.tmux" "$DIR/Cargo.toml" "$case_plugin/"
  tag="$(bash "$DIR/scripts/version.sh" tag)"
  case "$(uname -s):$(uname -m)" in
  Darwin:arm64) package=agenmux-macos-aarch64 ;;
  Darwin:x86_64) package=agenmux-macos-x86_64 ;;
  Linux:aarch64 | Linux:arm64) package=agenmux-linux-aarch64 ;;
  Linux:x86_64 | Linux:amd64) package=agenmux-linux-x86_64 ;;
  *)
...
RS
    ...
    cat >"$downloads/$tag/$package/target/release/agenmux" <<SH
#!/usr/bin/env bash
if [ "\${1:-}" = --version ]; then
  printf 'agenmux ${tag#v}\n'
elif [ "\${1:-}" = toggle ] && [ "\${2:-}" = popup ]; then
  printf opened >"$marker"
  sleep 0.3
fi
SH
    chmod +x "$downloads/$tag/$package/target/release/agenmux"
...
SH
  ...
  tmux -L "$socket" set-option -g @agenmux-popup-key e
  tmux -L "$socket" run-shell "bash '$case_plugin/agenmux.tmux'"
  ...
  for i in $(seq 1 600); do
    ...
    if [ "$mode" = bad-checksum ] && tmux -L "$socket" show-messages 2>/dev/null |
      grep -Fq 'agenmux: native engine installation failed'; then
      break
    fi
    sleep 0.1
  done
  ...
  if [ "$mode" = bad-checksum ]; then
    [ ! -e "$case_plugin/target/release/agenmux" ]
    [ ! -e "$marker" ]
    tmux -L "$socket" show-messages |
      grep -Fq 'agenmux: native engine installation failed'
  else
    [ "$opened" -eq 1 ]
    [ -x "$case_plugin/target/release/agenmux" ]
  fi
...
downloaded="$plugin/target/release/agenmux"
[ -x "$downloaded" ]
[ -s "$plugin/target/release/.agenmux-version" ]
# the sidebar's update notice and version picker read these
[ -s "$plugin/target/release/.agenmux-latest" ]
[ -s "$plugin/target/release/.agenmux-tags" ]
...
cp "$root/build/release/agenmux" "$plugin/target/source/agenmux"

phase=$SECONDS
run_tmux_case source "$plugin/target/source/agenmux"
